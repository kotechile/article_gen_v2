...
def main():
    ...
    try:
        ...
        if getattr(args, 'render_on_lambda', False):
            ...
            render_cmd = f"cd {remotion_dir} && npx remotion lambda render {serve_url} {comp_id} --region=us-east-1 --props=src/mockPayload.json --concurrency={args.concurrency} --timeout=900000 --function-name=remotion-render-4-0-484-mem3008mb-disk2048mb-900sec output-generated.mp4"
        else:
